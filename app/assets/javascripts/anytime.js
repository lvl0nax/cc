...
AnyTime.Converter = function(options)
{
	...
	this.fmt = '%Y-%m-%d %T';
	this.dAbbr = ['Вс','Пн','Вт','Ср','Чт','Пт','Сб'];
	this.dNames = ['Воскресенье','Понедельник','Вторник','Среда','Четверг','Пятница','Субота'];
	this.eAbbr = ['BCE','CE'];
	this.mAbbr = [ 'Янв','Фев','Мар','Апр','Май','Июн','Июл','Авг','Сен','Окт','Ноя','Дек' ];
	this.mNames = [ 'Январь','Февраль','Март','Апрель','Май','Июнь','Июль','Август','Сентябрь','Октябрь','Ноябрь','Декабрь' ];
	this.baseYear = null;
      ...
      this.id = 'AnyTime--'+id.replace(/[^-_.A-Za-z0-9]/g,'--AnyTime--');
        ...
        this.inp = $(document.getElementById(id)); // avoids ID-vs-pseudo-selector probs like id="foo:bar"
		    ...
		    this.inp.after(this.div);
		      ...
		      if ( askDoM )
		      {
		    	lab = options.labelDayOfMonth || 'Дни Месяца';
          ...
          var tensDiv, onesDiv;
		      ...
		      if ( askMinute )
		      {
		        ...
		        lab = options.labelMinute || 'Минуты';
            ...
            onesDiv = $('<ul class="AnyTime-mins-ones"/>');
		      ...
		      if ( askSec )
		      {
		        ...
		        lab = options.labelSecond || 'Секунды';
